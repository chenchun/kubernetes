...
func (kl *Kubelet) setNodeStatusUsedPorts(node *v1.Node) {
	...
	for j := range tcpData {
		...
		ports = append(ports, v1.UsedPort{Protocol: v1.ProtocolTCP, Port: int32(tcpData[j].Port)})
	}
	for j := range udpData {
		if udpData[j].State != "LISTEN" {
			continue
		}
		ports = append(ports, v1.UsedPort{Protocol: v1.ProtocolUDP, Port: int32(udpData[j].Port)})
	}
